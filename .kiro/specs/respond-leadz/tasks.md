...
- [x] 1. Establish database foundation, RLS, and shared types
  ...
  - [x] 1.2 Add RLS policies and FORCE ROW LEVEL SECURITY migration
...
- [x] 2. Implement startup configuration validation and structured logging
  - [x] 2.1 Implement ConfigValidator
  ...
  - [x] 2.2 Implement structured Logger with credential redaction
  ...
  - [x]* 2.3 Write property test for log credential redaction
    - **Property 25: Logs never contain credential values**
    - **Validates: Requirements 13.3**

  - [x]* 2.4 Write unit tests for ConfigValidator startup failures
    - Test missing/empty credential subsets produce named errors and refuse-webhook state
    - _Requirements: 1.7, 19.2_

- [x] 3. Implement SignatureVerifier
  - [x] 3.1 Implement HMAC-SHA256 signing and constant-time verification
  ...
  - [x]* 3.2 Write property test for signature round-trip and tamper rejection
    - **Property 2: Signature round-trip and tamper rejection**
    - **Validates: Requirements 3.1, 3.2, 3.3, 3.5, 3.6**

- [x] 4. Implement WebhookParser
  - [x] 4.1 Implement payload parsing, truncation, and name resolution
  ...
  - [x]* 4.2 Write property test for message extraction and count cap
    - **Property 3: Message extraction and count cap**
    - **Validates: Requirements 2.2, 2.4, 2.5**

  - [x]* 4.3 Write property test for message truncation
    - **Property 4: Message truncation is a bounded prefix**
    - **Validates: Requirements 2.3**

  - [x]* 4.4 Write property test for customer name defaulting
...
- [x] 5. Implement Tenant_Manager and RLS-scoped access
  - [x] 5.1 Implement tenant context, resolution, and RLS probe
  ...
  - [x]* 5.2 Write property test for tenant resolution from phone number id
    - **Property 24: Tenant resolution from phone number id**
    - **Validates: Requirements 12.5**

  - [x]* 5.3 Write property test for record tenant association
    - **Property 21: Every record is tenant-associated**
    - **Validates: Requirements 12.1**

  - [x]* 5.4 Write property test for tenant isolation on read and write
  ...
  - [x]* 5.5 Write property test for no-tenant-context denial
...
- [x] 7. Implement Inventory_Service
  - [x] 7.1 Implement tenant-scoped inventory search
  ...
  - [x]* 7.2 Write property test for tenant-scoped bounded inventory search
    - **Property 10: Inventory search is tenant-scoped and bounded**
    - **Validates: Requirements 6.2**

- [x] 8. Implement AI_Responder over a single LLM provider
  - [x] 8.1 Implement LlmProvider abstraction and inventory context rendering
  ...
  - [x] 8.2 Implement keyword extraction, response generation, token caps, and fallback
  ...
  - [x]* 8.3 Write property test for grounding only in returned inventory
    - **Property 11: Responses are grounded only in returned inventory**
    - **Validates: Requirements 6.3, 6.4**

  - [x]* 8.4 Write property test for referenced items including price and quantity
    - **Property 12: Referenced items always include price and quantity**
    - **Validates: Requirements 6.5, 6.6**

  - [x]* 8.5 Write property test for AI fallback on failure
    - **Property 14: AI failure produces a fallback**
    - **Validates: Requirements 8.1, 8.2, 8.3**

  - [x]* 8.6 Write property test for token caps on LLM requests
    - **Property 26: Token caps on LLM requests**
    - **Validates: Requirements 14.2**

- [x] 9. Implement Outbound_Sender
  - [x] 9.1 Implement WhatsApp send with retry policy
  ...
  - [x]* 9.2 Write property test for outbound send retry policy
    - **Property 13: Outbound send retry policy**
    - **Validates: Requirements 7.2, 7.3**

- [x] 10. Implement Conversation_Engine core (history, dedup, persistence)
  - [x] 10.1 Implement history fetch, append-and-trim, and persistence
  ...
  - [x] 10.2 Implement deduplication and conditional last-message-id write
  ...
  - [x]* 10.3 Write property test for deduplication yielding exactly one reply
    - **Property 6: Deduplication yields exactly one reply**
    - **Validates: Requirements 4.2, 4.4, 4.5, 14.3**

  - [x]* 10.4 Write property test for last-processed id set only after success
    - **Property 7: Last-processed id is set only after a successful reply**
    - **Validates: Requirements 4.3, 5.7**

  - [x]* 10.5 Write property test for history ordering, append, and bounded trim
    - **Property 8: History ordering, append, and bounded trim**
    - **Validates: Requirements 5.1, 5.3, 5.4**

  - [x]* 10.6 Write property test for conversation persistence round-trip
...
- [x] 12. Implement Close_Detector
  - [x] 12.1 Implement close evaluation and idempotent close-event recording
  ...
  - [x]* 12.2 Write property test for close detection totality
    - **Property 15: Close detection is total**
    - **Validates: Requirements 9.1**

  - [x]* 12.3 Write property test for idempotent close-event recording
    - **Property 16: Close-event recording is idempotent**
    - **Validates: Requirements 9.3, 9.4, 9.5**

- [x] 13. Implement consent, opt-out, and data deletion
  - [x] 13.1 Implement consent recording, opt-out, and deletion
  ...
  - [x]* 13.2 Write property test for data deletion removing personal data
    - **Property 20: Data deletion removes personal data**
    - **Validates: Requirements 18.3**

- [x] 14. Implement Lifecycle_Runner and daily cron
  - [x] 14.1 Implement follow-up scheduling and idempotent sending with consent gating
  ...
  - [x]* 14.2 Write property test for follow-up scheduling matching tenant plan
    - **Property 17: Follow-up scheduling matches the tenant plan**
    - **Validates: Requirements 10.1**

  - [x]* 14.3 Write property test for idempotent lifecycle sending
    - **Property 18: Lifecycle sending is idempotent**
    - **Validates: Requirements 10.2, 10.4**

  - [x]* 14.4 Write property test for consent and opt-out gating
    - **Property 19: Consent and opt-out gate follow-ups**
    - **Validates: Requirements 18.2, 18.4**

- [x] 15. Implement RateLimiter and queue
  - [x] 15.1 Implement burst queuing and spaced draining
  ...
  - [x]* 15.2 Write property test for burst messages queued, never dropped
    - **Property 27: Burst messages are queued, never dropped**
    - **Validates: Requirements 14.1, 15.3**

  - [x]* 15.3 Write property test for spaced queued sends
    - **Property 28: Queued sends are spaced**
    - **Validates: Requirements 14.4**

- [x] 16. Implement interop adapters
  - [x] 16.1 Implement CashFlow, Mails2Leadz, and AutoDealz adapters
  ...
  - [x]* 16.2 Write property test for lead handoff creating or updating one conversation
    - **Property 32: Lead handoff creates or updates one conversation**
    - **Validates: Requirements 16.2**

  - [x]* 16.3 Write unit tests for sibling outage resilience and CashFlow contract
    - Test sibling unavailability is logged and does not block inbound (16.5); CashFlow payload shape (16.1)
    - _Requirements: 16.1, 16.5_

- [x] 17. Wire the canonical Inbound_Handler endpoint
  - [x] 17.1 Implement GET verification challenge handler
  ...
  - [x] 17.2 Implement POST handler wiring signature, parse, tenant, rate limit, and per-message dispatch
  ...
  - [x]* 17.3 Write property test for webhook verification challenge
    - **Property 1: Webhook verification challenge**
    - **Validates: Requirements 1.2, 1.3, 1.4**

  - [x]* 17.4 Write property test for independent per-conversation processing
    - **Property 29: Independent per-conversation processing**
    - **Validates: Requirements 15.2**

  - [x]* 17.5 Write property test for webhook resilience returning 200 on processing error
    - **Property 30: Webhook resilience returns 200 on processing error**
    - **Validates: Requirements 17.3**

  - [x]* 17.6 Write property test for per-message log completeness
    - **Property 31: Per-message log completeness**
    - **Validates: Requirements 17.1**

- [x] 18. Implement health-check endpoint
  - [x] 18.1 Implement health endpoint reporting DB and WhatsApp reachability
...
- [x] 20. Consolidate to the single canonical implementation
  - [x] 20.1 Remove competing route and service variants
  ...
  - [x] 20.2 Archive non-production reference material
  ...
  - [x]* 20.3 Write structural consolidation test
